...
// Produces a sheet with all of the export information held on it.
func exportInfoSheet(sheet *xlsx.Sheet, e env.Env, f database.Filter) {
...
func studentDetails(row *xlsx.Row, s student.Student) {

	newCell(row, s.UPN, left)
	newCell(row, s.Name(), left)
	newCell(row, string(s.Gender), center)
